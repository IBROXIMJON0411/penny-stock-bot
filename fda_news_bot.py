...
def format_alert_for_article(article: dict, tickers: List[str], min_price: Optional[float]) -> str:
    url = article.get("article_url") or article.get("url") or article.get("canonical_url") or ""
    title_en = article.get("title") or article.get("headline") or ""
    summary_en = article.get("summary") or article.get("description") or ""

    # 🔁 TRANSLATE
    title_uz = translate_to_uz(title_en)
    summary_uz = translate_to_uz(summary_en[:500])  # узун бўлмасин
    ...
    published_raw = article.get("published_utc") or article.get("published_at") or ""
    dt = try_parse_date(published_raw)
    published_str = dt.astimezone().strftime("%Y-%m-%d %H:%M:%S") if dt else ""

    tickers_str = ", ".join(html.escape(t) for t in tickers)
    price_text = f"${min_price:.2f}" if min_price else "N/A"

    msg = (
        f"🥳 <b>{tickers_str}</b>  <code>{price_text}</code>\n\n"
        f"📰 <b>{html.escape(title_uz)}</b>\n\n"
        f"📌 <i>{html.escape(summary_uz)}</i>\n\n"
        f"Manba: {html.escape(source)}\n"
        f"Vaqt: {published_str}\n\n"
        f"{html.escape(url)}"
    )
    return msg
